...
async function main() {
  ...
  await write(
    "sitemap.xml",
    `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${["/", "/creators/", "/communities/", "/creator-studio/", "/subscriptions/", "/about/", "/privacy/", "/help/", ...records.map((r) => `/watch/${r.arweaveTx}/`)].map((route) => `<url><loc>${origin}${route}</loc></url>`).join("")}</urlset>`,
  );
  await write(
    "_headers",
