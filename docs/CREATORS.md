...
- `/moderators/` publicly lists wallets holding at least 1% of the $NIKKI supply (`NIKKI_MINT` in `wrangler.jsonc`). The Worker snapshots holders through `getTokenSupply`/`getProgramAccounts` on the configured RPC, caches the snapshot for five minutes at the edge, excludes a wallet matching `TREASURY_WALLET_HASH`, and joins the result against published channels on every request, so channel edits appear immediately. With no `NIKKI_MINT` configured the endpoint reports `enabled: false` and the page shows a configuration notice. The list describes token holdings, not an endorsement; founder moderation on `/ops/` remains separate.
- `/ops/` requires the exact server `FOUNDER_WALLET`, never a client role or X username. It is the founder admin panel, split into Overview, Channels, Communities, Posts, and Support sections: private support with replies, recent transactions, an audit trail, service checks, image capacity, and a persistent transaction pause switch, plus channel search with reversible unpublish, community search with reversible hiding (a hidden community's pages return 404 to everyone but its organizer and the founder), and a cross-site post feed with founder hiding. Every moderation action writes a `creator_ops_audit` row. Moderation never deletes rows or touches permanently published archive records. It checks existing transactions without creating or signing new ones. A missing original receipt still needs a separately verified operator recovery procedure; the status button does not attach or infer a signature.
